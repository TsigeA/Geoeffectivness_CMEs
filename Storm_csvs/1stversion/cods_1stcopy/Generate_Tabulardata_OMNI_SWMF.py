...
def build_feature_vector(omni_df: pd.DataFrame,
    ...
    for lag in range(n_lags + 1):
        ...
        if pd.isna(matched_time):
            return None

        #vals = omni_indexed.loc[t_lag]
        vals= omni_indexed.loc[matched_time]
...
def build_training_table(omni_df: pd.DataFrame,
    ...
    for t in target_df.index:
        # only use times before or at the peak such that forecast horizon also remains <= peak
        valid = True # flag to check if all future targets are valid (i.e., exist and are <= peak_time)
        target_vals = {}

        for h in horizons_min:
            t_future = t + pd.Timedelta(minutes=h)
            if t_future > peak_time:
                valid = False
                break
