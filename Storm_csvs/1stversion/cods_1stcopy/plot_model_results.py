...
storm_list_df = pd.read_csv('StormList.csv')
# read the csv files
tplus_60_df = pd.read_csv('validation_predictions_tplus_60m.csv')
tplus_120_df = pd.read_csv('validation_predictions_tplus_120m.csv')
...
for storm_id in unique_storm_ids:
    ...
    ax[0].plot(storm_df60['datetime'], storm_df60['y_pred'],color='orange', label='Predicted t= +60 minutes')
    ax2.plot(storm_df60['datetime'], storm_df60['residual'],'r', label='Residuals',linewidth=0.5,alpha=0.5)
    ...
    ax[0].vlines(storm_phase_info['SSC_Stime'], ymin=ymin60-1, ymax=ymax60+1, colors='red', linewidth=2, label='SSC Phase')
    ax[0].vlines(storm_phase_info['Main_Stime'], ymin=ymin60-1, ymax=ymax60+1, colors='green', linewidth=2, label='Main Phase')
    ax[0].vlines(storm_phase_info['Recov_Stime'], ymin=ymin60-1, ymax=ymax60+1, colors='magenta', linewidth=2, label='Recovery Phase')
    ...
    ax[1].plot(storm_df120['datetime'], storm_df120['y_pred'],color='orange', label='Predicted t= +120 minutes')
    ax2.plot(storm_df120['datetime'], storm_df120['residual'],'r', label='Residuals',linewidth=0.5,alpha=0.5)
    ax2.set_ylabel('Residuals [nT]', color='r', fontsize=14)
    ax2.tick_params(axis='y', labelcolor='r')
    ax2.set_ylim(-100, 100)
    ax2.grid(alpha=0.3, which='both')
    # ax[1].set_title(f'{storm_df120["datetime"].iloc[0]}: Predictions vs Actual Values for tplus 120', fontsize=16)
    ax[1].vlines(storm_phase_info['SSC_Stime'], ymin=ymin120-1, ymax=ymax120+1, colors='red', linewidth=2, label='SSC Phase')
    ax[1].vlines(storm_phase_info['Main_Stime'], ymin=ymin120-1, ymax=ymax120+1, colors='green', linewidth=2, label='Main Phase')
