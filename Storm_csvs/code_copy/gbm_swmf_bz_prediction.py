...
RUN_MANUAL_GRID_SEARCH = True # Set to True to perform manual grid search over PARAM_GRID, which will increase runtime significantly. If False, uses BASE_PARAMS directly.
...
def main() -> None:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"grouped_feature_importance_comparison_{current_time}.png")
    print(f"\nSaved grouped feature importance comparison plot to: grouped_feature_importance_comparison_{current_time}.png")
    # plot predictions vs actual values for each target and save to file
    
    for target_col, res in all_results.items():
        test_pred_df = res["val_predictions"] # use validation or test predictions as needed
        ...
        for storm_id in unique_storm_ids:
            ...
            fig.suptitle(f"{storm_df[DATETIME_COL].iloc[0]} Validation Results for {target_col}", fontsize=16)
            ...
            plt.tight_layout()
            plt.savefig(f"{output_dir}/{storm_df[DATETIME_COL].iloc[0]}_Validationresults_{target_col}_{current_time}.png")
            print(f"\nSaved predictions vs actual plot for {target_col} to: {output_dir}/{storm_df[DATETIME_COL].iloc[0]}_Validationresults_{target_col}_{current_time}.png")
