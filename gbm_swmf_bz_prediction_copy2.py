...
update:
    @May 1, 2026: Added kfold cross-validation across storms, with storm-wise splits in each fold.
       The previous single train/val/test split is replaced by kfold splits, and results are averaged across folds.
@author: TsigeA
@date: Apr 10, 2026
'''
...
def storm_kfold_cv(
    ...
    feature_cols: List[str],
    target_col: str,
    trainval_ids: List[str],   # storms available for CV (test set already removed)
    params: Dict,
    n_splits: int = 5,
    random_seed: int = RANDOM_SEED,
) -> List[Dict]:
    kf = KFold(n_splits=n_splits, shuffle=True, random_state=random_seed)
    storm_arr = np.array(trainval_ids)
    work = df.dropna(subset=[target_col]).copy()

    fold_results = []
    for fold, (train_idx, val_idx) in enumerate(kf.split(storm_arr)):
        fold_train_ids = storm_arr[train_idx].tolist()
        fold_val_ids   = storm_arr[val_idx].tolist()

        train_df = subset_by_storm_ids(work, fold_train_ids)
        val_df   = subset_by_storm_ids(work, fold_val_ids)

        X_train, X_val, _ = encode_feature_splits(train_df, val_df, val_df, feature_cols)
        y_train = train_df[target_col]
        y_val   = val_df[target_col]

        model = train_single_model(X_train, y_train, X_val, y_val, params)
        metrics = evaluate_regression(y_val.values, model.predict(X_val))
        metrics["fold"] = fold
        metrics["val_storm_ids"] = fold_val_ids
        fold_results.append(metrics)
        print(f"  Fold {fold}: RMSE={metrics['rmse']:.4f}  R²={metrics['r2']:.4f}")
...
def main() -> None:
    ...
    # Hold out a fixed test set — never touched during CV or hyperparameter tuning
    _, _, test_ids = split_storm_ids(storm_ids, train_frac=0.85, val_frac=0.0, test_frac=0.15)
    trainval_ids = [s for s in storm_ids if s not in test_ids]

    # Split trainval into train + val for final model (val used for XGBoost early stopping)
    rng = np.random.default_rng(RANDOM_SEED)
    tv = list(trainval_ids)
    rng.shuffle(tv)
    n_val = max(1, int(round(len(tv) * (VAL_FRAC / (TRAIN_FRAC + VAL_FRAC)))))
    val_ids = tv[:n_val]
    train_ids = tv[n_val:]
    ...
    for target_key in TARGETS_TO_RUN:
        ...
        # CV on trainval storms for unbiased model evaluation / hyperparameter selection
        print("\n--- 5-Fold Cross-Validation ---")
        storm_kfold_cv(df, feature_cols, target_col, trainval_ids, BASE_PARAMS, n_splits=5)

        # Final model: train on all trainval storms, evaluate on held-out test
