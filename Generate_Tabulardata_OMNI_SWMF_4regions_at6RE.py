...
2. Reads the SWMF Bz data and extracts the mean Bz at dayside_BZ_6RE, nightside_BZ_6RE, dawnside_BZ_6RE, duskside_BZ_6RE
3. Builds a training table with OMNI/ SW features and future Bz targets (per region) at specified forecast horizons,
ensuring that only rows with valid future targets up to the storm peak time are included.
4. Saves the resulting training table to a CSV file.
...
REGION_NAMES = ["dayside", "nightside","duskside", "dawnside"]
...
def extract_6RE_Bz_perregions(swmf_df: pd.DataFrame,
                                        target_radius_re: float = 6.0,
                                        tol: float =0.25) -> pd.DataFrame:
    """
    Extract Bz at 6 RE at 4 points: i.e., 0,6,12,24 hr or dayside, nightside, duskside and dawnside
    The 4 points are 
    X > 0 = dayside, X < 0 = nightside, Y > 0 = dusk, Y < 0 = dawn.
    dayside: x=6,y=0, nightside: x=-6,y=0, duskside: x=0, y=6, dawnside= x=0, y=-6
    Returns one row per datetime with target_Bz_{region}
    columns for each region in REGION_NAMES.
    """
    df = swmf_df.copy()
    df["r_re"] = np.sqrt(df["X_RE"] ** 2 + df["Y_RE"] ** 2)

    # mask = np.abs(df["r_re"] - target_radius_re) <= tol # work on the subset of the data
    # # subset = df[mask].copy()
    # mask.head()
    # if mask.empty:
    radius_mask = np.abs(df["r_re"] - target_radius_re) <= tol # work on the subset of the data
    subset = df[radius_mask].copy()
    if subset.empty:
        raise ValueError(
    ...
    #     )
    region_masks = {
        "dayside": (subset["X_RE"] > 0) & ((subset["Y_RE"]> -(0.125+2*R_TOL))&(subset["Y_RE"] < (0.125+2*R_TOL))),
        "nightside": (subset["X_RE"] < 0) & ((subset["Y_RE"]> -(0.125+2*R_TOL))&(subset["Y_RE"] < (0.125+2*R_TOL))),
        "duskside": ((subset["X_RE"]> -(0.125+2*R_TOL))&(subset["X_RE"] < (0.125+2*R_TOL))) & (subset["Y_RE"] > 0),
        "dawnside": ((subset["X_RE"]> -(0.125+2*R_TOL))&(subset["X_RE"] < (0.125+2*R_TOL))) & (subset["Y_RE"] < 0),
    }
    # region_masks = {
    #     "dayside": (subset["X_RE"] > 0),
    #     "nightside": (subset["X_RE"] < 0),
    #     "duskside": (subset["Y_RE"] > 0),
    #     "dawnside": (subset["Y_RE"] < 0),
    # }

    target = None
    for region in REGION_NAMES:
        region_subset = subset[region_masks[region]]
        ...
        region_target = (
            region_subset.groupby("datetime")
            .agg(**{
...
for i in range(len(OMNI_flist)):
    ...
    OUTPUT_CSV = f"{OMNI_FILE[10:-17]}_Bz_6RE_4regions.csv"
    ...
    #     prestorm_Stime = swmf_df.index.min()
        ...
        target_Qphase = extract_6RE_Bz_perregions(
            swmf_df=Quietphase,
    ...
    target_SSC = extract_6RE_Bz_perregions(
        swmf_df=SSCphase,
        target_radius_re=TARGET_RADIUS_RE,
        tol=R_TOL
    )
    target_SSC["phase"] = "SSC"
    target_Main = extract_6RE_Bz_perregions(
        swmf_df=Mainphase,
        target_radius_re=TARGET_RADIUS_RE,
        tol=R_TOL
    )
    target_Main["phase"] = "Main"
    target_Rec = extract_6RE_Bz_perregions(
        swmf_df=Recphase,
        target_radius_re=TARGET_RADIUS_RE,
        tol=R_TOL
    )
